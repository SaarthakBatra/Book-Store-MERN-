...
const CreateBook = () => {
    ...
    const handleSaveBook = () => {
        ...
        setLoading(true);
        axios.get('http://localhost:5555/books', data).then(() => {
            setLoading(false);
            navigate('/');
        }).catch((error) => {
            console.log(error);
        ...
        <div className='p-4'>
            ...
            <div className='flex flex-col border-2 border-sky-400 rounded-xl w-[600px] p-4 mx-auto'>
                ...
                <div className='my-4'>
                    ...
                    <input type='text' value={title} onChange={(e) => setAuthor(e.target.value)} className='border-2 border-grey-500 pc-4 py-2 w-full'></input>
                </div>
                <div className='my-4'>
                    <label className='text-xl mr-4 text-grey-500'>Publish Year</label>
                    <input type='text' value={title} onChange={(e) => setPublishYear(e.target.value)} className='border-2 border-grey-500 pc-4 py-2 w-full'></input>
                </div>
